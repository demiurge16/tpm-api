...
interface CountryClient {
    ...
    @RequestMapping(method = [RequestMethod.GET], value = ["/all?fields=name,cca3,currencies,flag,languages,capital,altSpellings"])
    fun getAll(): List<CountryExternalModel>

    @RequestMapping(method = [RequestMethod.GET], value = ["/alpha/{code}?fields=name,cca3,currencies,flag,languages,capital,altSpellings"])
    fun getByCode(@PathVariable(name = "code", required = true) code: String): CountryExternalModel?

    @RequestMapping(method = [RequestMethod.GET], value = ["/name/{name}?fields=name,cca3,currencies,flag,languages,capital,altSpellings"])
